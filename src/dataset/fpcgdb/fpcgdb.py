...
_subpath = Path('./physionet.org/files/fpcgdb/1.0.0')


def _get_file_list(conf: Configuration) -> List[str]:
    file_list = glob(str(conf.paths['fpcgdb'] / _subpath / '*.dat'))
    file_list.sort()

    return file_list


def create_fpcgdb_window_lists(conf: Configuration):
    print('Loading FPCGDB dataset')
    ...
    for file in file_list:
        ...
        if window_l.__len__() > 0:
            fpcgdb_window_lists.append(window_l)

    return fpcgdb_window_lists


if __name__ == '__main__':
    # Example of using this dataset
    conf = Configuration(Path('./configuration/config.yml'))
    window_lists = create_fpcgdb_window_lists(conf)
